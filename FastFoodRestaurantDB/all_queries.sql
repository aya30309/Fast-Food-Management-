...
BEGIN
    ...
    GROUP BY o.OrderId, o.OrderNo, o.UserId, o.Status, o.OrderDate;
END;
...
----------------------------------------------------------------------
--2. Triggers
---1. Trigger to Update Product Stock After an Order is Placed: كمية المخرزون 
...
CREATE TRIGGER UpdateOrderStatusAfterInsert
ON Orders
AFTER INSERT
AS
BEGIN
    -- Update the order status to 'Completed' for the newly inserted order(s)
    UPDATE Orders
    SET Status = 'Completed'
    WHERE OrderId IN (SELECT OrderId FROM inserted);

    -- Optional: Output message for debugging (will show up in SSMS)
